-- infrastructure/postgres/migrations/streams_db/000003_create_fdw_users.up.sql
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '✅ JOIN test completed (no streams yet)';
  END IF;

  RAISE NOTICE '✅ Migration 000003 completed successfully';
  RAISE NOTICE '📝 Stream Service can now: SELECT s.*, u.username FROM streams s LEFT JOIN users u ON s.user_id = u.id';
